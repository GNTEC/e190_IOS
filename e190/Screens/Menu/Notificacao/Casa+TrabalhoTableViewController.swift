...
class Casa_TrabalhoTableViewController: UITableViewController {
        ...
        if self.text_cep_casa.text != ""{
            ...
            api.buscaEnderecoPorCep(cep: cepSemMascara, completionHandler: { (retorno) in
                ...
                if retorno["erro"] != nil {
                   ...
                   self.indicator!.stop()
                   self.present(self.alerta.alertaSimples(titulo: "Alerta", mensagem: "Endereço não encontrado !"), animated: true, completion: nil)
                }
                else
                {
    ...
    func buscarEndereco()
    {
        ...
        {
            api.listar(codUsuario: codUsuario, completationHandler: { (objUsuario) in
                
                //VERIFICA SE EXISTE ENDEREÇO TRABALHO E CASA
                if objUsuario["enderecoCasa"] as! String != "" || objUsuario["enderecoCasa"] as? String  != nil {
                    
                    self.text_cep_casa.text = objUsuario["cepCasa"] as? String
                    self.text_end_casa.text = objUsuario["enderecoCasa"] as? String
                    self.atualizarDados = true
                }
                
                if objUsuario["enderecoTrabalho"] as! String != "" || objUsuario["enderecoTrabalho"] as? String != nil {
                    
                    self.text_cep_trabalho.text = objUsuario["cepTrabalho"] as? String
